...
Meteor.methods({
  createActivity: function(activity, tweet, loc) {
    ...
    activity.userAvatar = 'https://avatars3.githubusercontent.com/u/204768?v=2&s=400',
    activity.userName = (Meteor.user().profile)? Meteor.user().profile.name : Meteor.user().emails[0].address;
...
if (Meteor.isServer) {
  ...
  var getLocationPlace = function(loc) {
    console.log('loc.coords', loc, loc.coords);
